...
from model import FuzzingPlatform
from model.FuzzingPlatform import FuzzingPlatform
from app import app

db = app.config['db']

class PlatformCtrl(Resource):
    def create(self, data):
        try:
            platform = FuzzingPlatform(data['name'])
            db.session.add(platform)
            db.session.commit()
        except Exception as e:
            return {"err" : "invalid request"}, 400

        return platform.as_dict(), 201

    def read(self, id):
        platform = FuzzingPlatform.query.filter_by(id=id).first()
        if platform is None:
            return {"err": "not found"}, 404
        return platform.as_dict(), 200
    ...
    def list(self, offset=0, limit=10000):
        platforms = FuzzingPlatform.query.offset(offset).limit(limit).all()
        platforms = [plat.as_dict() for plat in platforms]
